...
class NavLiftController(Node):
    ...
    def run(self):
        init_pose = self.make_pose(-10.0, -21.1, 0.0)
        ...
        pose1 = self.make_pose(-1.8, -8.0, 1.83)
        pose2 = self.make_pose(-1.8, -6.2, 1.83)
        ...
        self.back_up(speed=-0.8, duration=7.0)
        time.sleep(1.0)

        self.get_logger().info("천천히 회전 시작")
        self.rotate_slowly(angular_speed=0.8, duration=2.5)
        time.sleep(2.0)

        self.get_logger().info("두번째 이동 시작")
        final_pose = self.make_pose(-11.9, 22.0, 0.0)
        ...
        self.back_up(speed=-0.6, duration=8.0)
        time.sleep(1.0)
